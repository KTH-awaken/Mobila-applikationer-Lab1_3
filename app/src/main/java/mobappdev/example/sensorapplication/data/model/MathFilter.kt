...
class MathFilter {
    ...
    fun calculateAnglesWithGyro(linSample:Triple<Float,Float,Float>,gyroSample:Triple<Float,Float,Float>):Triple<Float,Float,Float>{
        val xAngleRaw = atan2(linSample.second, linSample.third) * (360 / Math.PI)
        val yAngleRaw = atan2(linSample.first, linSample.third) * (360 / Math.PI)
        val zAngleRaw = atan2(sqrt(linSample.first * linSample.first + linSample.second * linSample.second), linSample.third) * (180 / Math.PI)

        val xGyroAngleRaw = atan2(gyroSample.second, gyroSample.third) * (360 / Math.PI)
        val yGyroAngleRaw = atan2(gyroSample.first, gyroSample.third) * (360 / Math.PI)
